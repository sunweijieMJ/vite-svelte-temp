<div class="login">
    ...
    <form class="login-form" onsubmit="return false">
        <h3 class="title">{ i18n.t('baseLogin.t1') }</h3>
        <div class="form-item account">
            <label class="form-item-label" for="login-form-account">{ i18n.t('baseLogin.t2') }</label>
            <input class="form-item-input" id="login-form-account" bind:value={loginForm.account} maxlength="50" />
            <p class="form-item-error"></p>
        </div>
        <div class="container">
            <div class="form-item password">
                <label class="form-item-label" for="login-form-password">{ i18n.t('baseLogin.t3') }</label>
                <input class="form-item-input" id="login-form-password" bind:value={loginForm.password} type="password" autocomplete maxlength="50" />
                <p class="form-item-error"></p>
            </div>
            <div class="form-item code">
                <label class="form-item-label" for="login-form-code">{ i18n.t('baseLogin.t4') }</label>
                <input class="form-item-input" id="login-form-code" bind:value={loginForm.captchaCode} maxlength="4" />
                <p class="form-item-error"></p>
                <img class="graphics" src="{captchaImage}" alt="" on:click="{getGraphicCode}" />
            </div>
    ...
    const handleLogin = async () => {
        await basicApi.authLogin({
            ...
            if (res.status) {
                ...
                // this.$message({ type: 'success', message: `${this.$t('baseLogin.t111')}` });
            } else {
                getGraphicCode();
            }
        });
