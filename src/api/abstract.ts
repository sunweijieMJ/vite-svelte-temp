    ...
    private apiAxios({
        ...
        return new Promise((resolve, reject) => {
            instance({
                ...
                // 200:服务端业务处理正常结束
                const message = res.data?.errorMessage || (`${url}${i18n.t('baseAbstract.t0')}`);
                if (res.status === 200) {
                    if (res.data.success) {
                        resolve({
                ...
                switch (err.status) {
                    case 401:
                        message = `${i18n.t('baseAbstract.t1')}`;
                        break;
                    default:
                        message = err?.data?.errorMessage || err?.message || (`${url}${i18n.t('baseAbstract.t0')}`);
                        break;
                }
                // CustomToast({ message });
